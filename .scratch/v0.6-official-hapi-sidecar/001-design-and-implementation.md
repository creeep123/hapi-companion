...
- A content-free event-shape probe found frequent `session-updated` metadata wrappers during active work. Their nested values contain product-relevant fields such as name, lifecycle state, machine and flavor, so alpha.8 conservatively refreshes detail/catalog instead of discarding them. Further optimization would require another inference/cache layer. Per the one-candidate stop rule, no alpha.9 was created.
