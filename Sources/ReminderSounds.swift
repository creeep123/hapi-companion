...
struct ReminderSoundPreset: Identifiable {
    ...
    static let all: [Self] = [
        .init(id: "original", name: "经典完成（原提示音）", resource: "HapiComplete", ext: "aiff"),
        .init(id: "confirmation", name: "轻快确认", resource: "SoundConfirmation", ext: "wav"),
        .init(id: "glass", name: "玻璃轻铃", resource: "SoundGlass", ext: "wav"),
        .init(id: "pluck", name: "短促拨弦", resource: "SoundPluck", ext: "wav"),
        .init(id: "pixel", name: "像素升级", resource: "SoundPixel", ext: "wav"),
        .init(id: "digital", name: "电子三音", resource: "SoundDigital", ext: "wav")
    ]
}
...
final class ReminderSounds {
    ...
    @discardableResult
    func play(using start: ((URL) -> Bool)? = nil) -> Bool {
        let start = start ?? { [self] url in
        ...
        if let chosen, start(chosen) { feedback = nil; return true }
        if saved.selected != "original", let fallback = presetURL("original"), start(fallback) {
