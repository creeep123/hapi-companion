...
{"version":"0.4.0","architecture":"$ARCH","sha256":"$HASH","stateSchema":1,"managementApi":1}
EOF
tar -C dist/bundle -czf "dist/hapi-mobile-relay-0.4.0-$ARCH.tar.gz" .
SMOKE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hapi-relay-bundle.XXXXXX")"
trap 'rm -rf "$SMOKE_DIR"' EXIT
tar -C "$SMOKE_DIR" -xzf "dist/hapi-mobile-relay-0.4.0-$ARCH.tar.gz"
(cd /tmp && "$SMOKE_DIR/smoke-bundle.sh" "$SMOKE_DIR")
shasum -a 256 "dist/hapi-mobile-relay-0.4.0-$ARCH.tar.gz"
