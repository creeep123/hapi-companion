...
if (command === 'pair-code') {
  ...
  if (command === 'serve-sidecar') {
    ...
    const token = await readSourceToken(); const namespace = token.includes(':') ? token.slice(token.lastIndexOf(':') + 1) : 'default'; const namespaceHash = createHash('sha256').update(namespace).digest('hex')
    ...
    const source = new SidecarSourceEngine(sidecarStore, new EventInterpreter(publicOrigin, namespaceHash), broker, new OfficialHapiClient(hapiOrigin, token), undefined, () => ntfy.signal())
    const noLegacyEngine = { start: async () => {}, stop: async () => {}, restart: async () => {} } as RelayEngine
    const sidecarRelayManager = new RelayManager(store, noLegacyEngine), sidecarManager = new SidecarManager(sidecarRelayManager, sidecarStore, publicOrigin, apiOrigin)
    handler = createSidecarHandler(broker, sidecarManager, createHandler(sidecarRelayManager)); start = async () => { ntfy.start(); source.start() }; stop = async () => { ntfy.stop(); await source.stop(); sidecarStore.close() }
  }
...
} else { process.stderr.write('usage: hapi-mobile-relay [serve|serve-sidecar|pair-code]\n'); process.exit(2) }
