...
function validateState(value: PersistedState): void {
  ...
  if (value.enabled && (!value.config || (value.sourceMode !== 'officialHapi' && (!value.credential || value.activation?.status !== 'committed')))) throw new Error('incomplete active state')
