...
function validateState(value: PersistedState): void {
  ...
  if (value.sourceMode !== undefined && !['patchedHub', 'officialHapi'].includes(value.sourceMode)) throw new Error('invalid source mode')
  if (value.officialCutoverAt !== undefined && (!Number.isSafeInteger(value.officialCutoverAt) || value.officialCutoverAt < 0 || value.sourceMode !== 'officialHapi')) throw new Error('invalid official cutover')
  if (value.enabled && (!value.config || (value.sourceMode === 'officialHapi' ? value.officialCutoverAt === undefined : (!value.credential || value.activation?.status !== 'committed')))) throw new Error('incomplete active state')
