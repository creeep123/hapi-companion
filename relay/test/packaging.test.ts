...
const sidecarInstaller = resolve(import.meta.dir, '../../scripts/install-sidecar.sh')
describe('Linux installer validation', () => {
  ...
  test.each(['..', '../1.2.3', '01.2.3', '1.02.3', '1.2.03', '1.2', '1.2.3-..'])('Sidecar installer rejects unsafe/non-semver version %s', async version => {
    const process = Bun.spawn([sidecarInstaller, '/bin/ls', version, 'a'.repeat(64)], { stdout: 'pipe', stderr: 'pipe' })
    expect(await process.exited).toBe(2); expect(await new Response(process.stderr).text()).toContain('strict semver')
  })
