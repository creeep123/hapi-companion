...
final class SessionOpenerTests: XCTestCase {
    func testSameOriginIgnoresPath() throws {
        ...
        XCTAssertTrue(SessionOpener.sameOrigin(pwa, session))
    }

    func testSameOriginRejectsDifferentPortOrHost() throws {
        let origin = try XCTUnwrap(URL(string: "https://hapi.example/"))
        XCTAssertFalse(SessionOpener.sameOrigin(try XCTUnwrap(URL(string: "https://other.example/")), origin))
        XCTAssertFalse(SessionOpener.sameOrigin(try XCTUnwrap(URL(string: "https://hapi.example:8443/")), origin))
    }
}
