...
Existing `/health`, `/v1/pair`, `/v1/status`, `/v1/config`, `/v1/test`, pause, resume and removal semantics remain compatible for the mobile UI. The Sidecar runtime rejects legacy patched-Hub activation and repair with an upgrade-required response because it has no patched source engine. Add management-authenticated operations to create and revoke Mac consumers. Issued bearer values are returned once.
...
- `/v1/activate` and `/v1/repair` return `409 {error:"client_upgrade_required",requiredAPI:2}` from the Sidecar runtime; the separately deployed legacy Relay keeps its v1 behavior during rollback;
...
- startup authenticates, derives the namespace binding from the returned HAPI JWT, loads catalog and reaches the SSE connected frame before the source becomes live; v2 activation requires that live state before committing `sourceMode=officialHapi`.
...
`/v2/status` exposes only `{version:2,source:{state,attentionCode?,updatedAt},delivery:{enabled,cutoverAt}}` to the management credential. `/v1/status` retains its mobile-compatible response. Neither response contains source credentials, consumer tokens or the ntfy topic.
