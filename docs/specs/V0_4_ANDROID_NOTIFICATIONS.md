...
2. The app shows a QR containing the documented `ntfy://ntfy.sh/<topic>` deep link and a “复制主题名称” fallback. The user may scan with the system camera or paste only the copied topic into ntfy's default-server “主题名称” field. QR recognition is a required real-device acceptance item because the earlier experiment used manual topic entry; the copy path must work independently.
