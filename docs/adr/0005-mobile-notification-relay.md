...
The Relay processes one event at a time in sequence. It durably records provider acceptance or intentional suppression before ACK; any failed head event stops later processing. It always rebuilds the click URL from the immutable configured HAPI HTTPS origin and encoded session ID. Public ntfy receives fixed product text rather than session titles or response content.

V0.4 supports one phone and public ntfy.sh by default. The base URL remains configurable for later self-hosting. Send only a title, fixed completion text and validated same-origin HAPI session URL.

## Consequences

Mobile delivery continues while the Mac sleeps and preserves the existing durable SSE/ACK model without changing the HAPI patch or updater pin. The VM gains one small service and an HTTPS management surface that require packaging, health checks, upgrade/rollback and credential handling. Rule changes synchronize explicitly from one controlling Mac with revisions and visible split state. Delivery is at least once: exactly-once handset display remains impossible across ntfy's accepted-request boundary.
